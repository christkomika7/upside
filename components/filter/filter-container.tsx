...
export default function FilterContainer({
  ...
  const bed = useHeroFilters.use.bedroom();
  const bath = useHeroFilters.use.bathroom();
  ...
  return (
    <div className="relative gap-x-8 gap-y-4 grid grid-cols-1 md:grid-cols-[1fr_130px] bg-white/20 md:bg-white backdrop-blur-2xl p-4 md:p-2 rounded-[32px] md:rounded-full w-full h-fit md:h-16">
      <div className="relative w-full h-full overflow-hidden">
        {showLeftArrow && (
          <button
            onClick={scrollLeft}
            className="top-1/2 left-0 z-10 absolute flex justify-center items-center bg-white/80 shadow-md backdrop-blur-xl rounded-full w-6 h-6 -translate-y-1/2 cursor-pointer"
          >
            <ChevronLeft size={16} />
          </button>
        )}
        ...
        <motion.ul
          ref={scrollRef}
          className="flex md:flex-row flex-col gap-x-1 gap-y-2 rounded-none md:rounded-full w-full h-fit md:h-full overflow-x-auto scroll-smooth scrollbar-hide"
          initial={false}
          animate={{ x: 0 }}
        ...
        >
          <li
            ...
            className={clsx(
              "flex justify-center items-center gap-x-2 bg-white md:bg-transparent hover:bg-(--brown)/20 backdrop-blur-2xl px-5 py-5 md:py-0 border-neutral-300 rounded-full h-fit md:h-full font-medium text-sm text-left cursor-pointer shrink-0",
              rubik.className,
          ...
          {bed && (
            <motion.li
              ...
              className={clsx(
                "hidden md:flex justify-center items-center gap-x-2 !bg-(--brown) hover:bg-(--brown)/20 backdrop-blur-2xl px-5 border-neutral-300 rounded-full h-full font-medium text-sm text-left cursor-pointer shrink-0",
                rubik.className
              )}
            >
              {bed} {t("bedroom")}
        ...
        {showRightArrow && (
          <button
            onClick={scrollRight}
            className="top-1/2 right-0 z-10 absolute flex justify-center items-center bg-white/80 shadow-md backdrop-blur-xl rounded-full w-6 h-6 -translate-y-1/2 cursor-pointer"
          >

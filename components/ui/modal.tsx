...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    className={cn(
      "fixed inset-0 z-[3000] bg-black/20 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
    ref={ref}
...
const ResponsiveModalVariants = cva(
  cn(
    "fixed gap-4 border-white bg-white rounded-xl shadow-sm p-4 transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500 overflow-y-auto",
  ...
  ),
  {
    variants: {
      side: {
        ...
        bottom:
          "inset-x-0 bottom-0 border-t sm:h-fit max-h-fit rounded-t-xl data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
...
type ExtensionResponsiveModalContentProps = {
  hasIcon?: boolean;
} & ResponsiveModalContentProps;

const ResponsiveModalContent = React.forwardRef<
  React.ElementRef<typeof DialogPrimitive.Content>,
  ExtensionResponsiveModalContentProps
>(({ side = "bottom", hasIcon = true, className, children, ...props }, ref) => (
  <ResponsiveModalPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        className,
        "z-[3000] rounded-b-none sm:rounded-b-xl "
      )}
      {...props}
    >
      {children}
      {hasIcon && (
        <ResponsiveModalClose className="top-4 right-4 absolute data-[state=open]:bg-secondary opacity-70 hover:opacity-100 rounded-sm focus:outline-none ring-offset-background transition-opacity disabled:pointer-events-none">
          <X className="w-4 h-4" />
